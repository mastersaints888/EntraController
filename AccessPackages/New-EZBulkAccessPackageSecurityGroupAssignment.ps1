function New-EZBulkAccessPackageSecurityGroupAssignment {
$ErrorActionPreference = 'Stop'
Start-Process -FilePath "$ENV:USERPROFILE\Documents\EntraController\AccessPackages\New-EZBulkAccessSecurityGroupAssignment.csv"
Write-Host "Attempting to open CSV. Please input the Catalog Name, the AccessPackage Name and the Entra group name in which you want to add to each. Click save and exit when finished"
...
#user prompt
    while (-not $Confirmed) {
        ...
        try {
            ...
            $P = Read-Host "Type Y when finished, C to dry run (or type Q to quit)"
            ...
            switch ($P.ToUpper()) {
                ...
                "C" {
                    $ResourceNames = Import-Csv -Path "$ENV:USERPROFILE\Documents\EntraController\AccessPackages\New-EZBulkAccessSecurityGroupAssignment.csv"
                        ...
                        Write-Host "[DRYRUN] Would apply Group '$($Resource.GroupName)' to Access Package '$($Resource.AccessPackageName)' in Catalog '$($Resource.CatalogName)'" -ForegroundColor Yellow
                        }                    
                }
                "Q" {
                ...
                default {
                    Write-Host "Invalid input. Please type 'Y' to continue 'C' to dryrun or 'Q' to quit." -ForegroundColor Red
                }
...
# Catalog, Access Package and Group ID names grabbed below
...
foreach($Group in $ResourceNames){
    ...
    Write-Host "Successfully added group $($Group.GroupName) to Access Package $($Group.AccessPackageName)." -ForegroundColor Green
    }
    catch{ Write-Host "Failed to add group $($Group.GroupName) to Access Package." $_ -ForegroundColor Red
    }
}
